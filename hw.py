# wanted to use session expiry but not working
...
async def start():
    ...
    cl.user_session.set("last_active_time", time.time())
    # await cl.Message(
    #   content="Welcome! Your session will expire after 10 sec of inactivity."
    # ).send()
...
async def main(message: cl.Message):
    ...
    last_active_time = cl.user_session.get("last_active_time")
    if current_time - last_active_time > SESSION_TIMEOUT:
        await cl.Message(
            content="⏱️ Your session has expired due to inactivity. Please refresh or restart the chat."
        ).send()
        # Optionally, clear session data
        cl.user_session.clear()
        return
    cl.user_session.set("last_active_time", current_time)

    # Normal message handling
    await cl.Message(content=f"Session Refreshed: {message.content}").send()
